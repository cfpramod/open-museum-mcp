...
- **Colour extraction now runs on Wellcome Collection records.** The CDN allowlist gating which image hosts may be fetched for colour analysis never included `iiif.wellcomecollection.org`, and because that path fails open, every Wellcome record silently received no `dominantColor` or `colorFamily`. Dominant-colour search had been blind to the source since it was added. Verified live against real records after the fix.
