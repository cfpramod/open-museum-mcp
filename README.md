...
If anyone else is exploring open-access art, I hope this helps. The plan is to keep adding museums from around the world — see [`docs/coverage-roadmap.md`](docs/coverage-roadmap.md) for the current queue.
...
| Wikimedia Commons | `imageinfo[0].extmetadata.License.value` (string) | `=== "cc0"`, or a worldwide PD token (`pd`, `pd-art`, `pd-old*`, `pdm`, `pdm-*`); jurisdiction-scoped tokens (`pd-us*`, `pd-1923`, …) are rejected |
...
| Walters Art Museum | dataset-level CC0 declaration (no per-object rights field) | the museum publishes its whole released dataset under CC0; we accept only records that are **pre-1928** (latest date `< 1928`) **and image-bearing**, and the build-time ingest additionally excludes loan-flagged and copyright-mentioning records. The 1928+/loaned tail is dropped rather than guessed |
| SMK (National Gallery of Denmark) | `public_domain` (boolean) + `rights` (URI) | `=== true`; the `rights` URI tiers the license (CC0 vs Public Domain Mark) |
| Wellcome Collection | the `iiif-image` location's `license.id` | `=== "cc0"` or `"pdm"` (CC-BY and everything else rejected); library records are additionally curated to visual-art `workType`s |
| National Gallery of Art (Washington) | `published_images.openaccess` (CSV flag) | `=== "1"` on the primary image (NGA's CC0 open-access programme); image-bearing required |
| Harvard Art Museums | `imagepermissionlevel` (integer) | `=== 0` (open access). Harvard publishes no CC0/PD declaration, so the rights are surfaced honestly as `OTHER` / open-access — not promoted to CC0/PD. Records are never cached (Harvard ToS) |
| J. Paul Getty Museum | two independent checks: the object's own `subject_to` (collection metadata), and the shown image's own media-entity `subject_to` (Open Content Program) | both must classify as the CC0 URI to accept an open, embeddable image; metadata-only records (object CC0, image not) are still accepted, with `imageOpenAccess: false` and no `imageUrls.full` — never inherited from one level to the other |
...
- `license.type`: normalized license tier (`CC0`, `PD`, `CC-BY`, `OTHER`, …). Most sources emit `CC0` or `PD`; a source with an open-access permission that isn't a standard CC licence (e.g. Harvard's `imagepermissionlevel=0`) is surfaced as `OTHER` with `imageOpenAccess: true`, rather than over-claimed as CC0/PD — the rights are shown faithfully, the Open Clearance way.
...
| Wellcome Collection | `wellcome` | none | ✅ v0.14 |
| National Gallery of Art (Washington) | `nga` | none (bundled CC0 dataset) | ✅ v0.15 |
| Harvard Art Museums | `harvard` | API key (free) | ✅ v0.15 |
| J. Paul Getty Museum | `getty` | none | ✅ v0.17 — two-tier rights: collection metadata is CC0, but each image's rights are verified independently (~91k of ~168k objects have an Open Content Program image; the rest are metadata-only) |
